...
const fetchIssues = (workspaceUrl, email, apiKey) => {
    ...
    return fetch(`${workspaceUrl}/rest/api/2/search?jql=${query}&fields=${fields}`, { headers })
        .then((res) => res.json())
        .then((json) =>
...
exports.activate = async (context) => {
    ...
    let apiKey = null
    if (config.get('user.apiKeyCmd')) {
        const { err, stdout, stderr } = await exec(config.get('user.apiKeyCmd'))
        if (err) {
            workspace.showMessage('Failed to fetch password from command', 'error')
            return
        }
        if (stderr) {
            workspace.showMessage('apiKeyCmd wrote text to stderr', 'warn')
        }
        apiKey = stdout
    } else {
        apiKey = config.get('user.apiKey') || process.env.JIRA_USER_API_KEY
    }

    if (!workspaceUrl || !email || !apiKey) {
        workspace.showMessage('JIRA configuration missing from :CocConfig', 'warning')
        return
    }
    ...
    try {
        ...
        workspace.showMessage('Failed to fetch JIRA issues, check :CocOpenLog', 'error')
